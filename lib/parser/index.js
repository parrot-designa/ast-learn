...
function parse(template, options) {
  ...
  function closeElement(element) {
    ...
    if (currentParent) {
      if (element.elseif || element["else"]) {
        (0, _directive.processIfConditions)(element, currentParent);
      } else {
  ...
  (0, _htmlParser.parseHTML)(template, {
    start: function start(tag, attrs, unary) {
       ...
       */
      if (!inVPre) {
        (0, _directive.processPre)(element);
        if (element.pre) {
          inVPre = true;
        }
      }
      ...
      if (inVPre) {
        (0, _attrs.processRawAttrs)(element);
      } else {
        // 不是在v-pre下 所以需要先进行指令的处理
        (0, _directive.processFor)(element);
        (0, _directive.processIf)(element);
        (0, _directive.processOnce)(element);
      }
      if (!root) {
        /**
    ...
    chars: function chars(text, start, end) {
      ...
      if (text) {
        var child, res;
        // 存在插值表达式
        if (!inVPre && text !== ' ' && (res = (0, _textParser.parseText)(text))) {
          child = {
            type: 2,
