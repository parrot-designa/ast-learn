...
function parseHTML(html) {
  ...
  while (html) {
    ...
    if (textEnd === 0) {
       ...
       *  匹配  </...>  结束标签 必须以 </ 开始
       */
      var endTagMatch = html.match(_reg.endTag);
      if (endTagMatch) {
        // 在截取字符串前保存当前下标
        var curIndex = index;
  ...
  function parseStartTag() {
    var start = html.match(_reg.startTagOpen);
    if (start) {
      var match = {
        tagName: start[1],
        attrs: [],
      ...
      while (
      // 匹配不到开始标签对应的闭合标签 > 
      !(end = html.match(_reg.startTagClose))
      // 匹配到属性 或者动态属性
      && (attr = html.match(_reg.dynamicArgAttribute) || html.match(_reg.attribute))) {
  ...
  function parseEndTag(tagName, start, end) {
    ...
    if (pos >= 0) {
      ...
      /**
       * stack数组用于跟踪未闭合的开始标签，每个开始标签被添加到栈顶，而当遇到相应的结束标签时，栈顶的开始标签应该被弹出，以保证标签的正确嵌套。
       * pos变量表示的是需要保留的栈底位置，也就是从pos到栈顶的所有元素都是需要被“关闭”的开放标签。这部分代码的目的就是处理这些开放标签，确保它们都有对应的结束标签，或者在没有对应结束标签的情况下给出警告，并最终更新stack使其只包含那些不需要关闭的标签。
       * 将stack.length设置为pos的作用是裁剪栈，移除所有在pos索引之上的元素。这是因为这些元素代表的开始标签已经被处理过了，要么是因为找到了匹配的结束标签，要么是因为已经发出了缺少结束标签的警告。裁剪栈可以保证栈中只包含真正未闭合的标签，这样在后续的解析过程中，栈的内容就能准确地反映当前未闭合标签的情况。
       */
      stack.length = pos;
    }
  }
  function handleStartTag(match) {
    ...
    // 是否是自闭合标签
    var unary = (0, _util.isUnaryTag)(tagName) || !!unarySlash;
    // 获取属性的数量
