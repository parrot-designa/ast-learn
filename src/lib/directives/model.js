...
var _events = require("../parser/events");
...
function genDefaultModel(el, value) {
  ...
  (0, _props.addProp)(el, 'value', "(".concat(value, ")"));
  (0, _events.addHandler)(el, event, code, null);
}
function genSelect(el, value) {
  ...
  (0, _events.addHandler)(el, 'change', code, null);
}
...
function genCheckboxModel(el, value) {
  ...
  (0, _props.addProp)(el, 'checked', "Array.isArray(".concat(value, ")") + "?_i(".concat(value, ",").concat(valueBinding, ")>-1") + (trueValueBinding === 'true' ? ":(".concat(value, ")") : ":_q(".concat(value, ",").concat(trueValueBinding, ")")));
  (0, _events.addHandler)(el, 'change', "var $$a=".concat(value, ",") + '$$el=$event.target,' + "$$c=$$el.checked?(".concat(trueValueBinding, "):(").concat(falseValueBinding, ");") + 'if(Array.isArray($$a)){' + "var $$v=".concat(valueBinding, ",") + '$$i=_i($$a,$$v);' + "if($$el.checked){$$i<0&&(".concat(genAssignmentCode(value, '$$a.concat([$$v])'), ")}") + "else{$$i>-1&&(".concat(genAssignmentCode(value, '$$a.slice(0,$$i).concat($$a.slice($$i+1))'), ")}") + "}else{".concat(genAssignmentCode(value, '$$c'), "}"), null);
}
function genRadioModel(el, value) {
  var valueBinding = (0, _helpers.getBindingAttr)(el, 'value') || 'null';
  valueBinding = valueBinding;
  (0, _props.addProp)(el, 'checked', "_q(".concat(value, ",").concat(valueBinding, ")"));
  (0, _events.addHandler)(el, 'change', genAssignmentCode(value, valueBinding), null, true);
}